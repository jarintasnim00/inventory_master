...
<i style="cursor: pointer;" id="delete_button_' . $count . '" onclick="delete_row(' . $count . ')" class="fa fa-trash"></i>
    ...
    public function update_purchase()
    {
        ...
        $where = array('purchase_no' => $custid);
        $this->load->model('Main_model');
        $response = $this->Main_model->update_record('purchase_company', $cust_info, $where);
        if ($response) {
            $this->session->set_flashdata('msg', 'Data Updated Successfully..!');
            redirect(base_url() . "index.php/purchase/purchase_history");
        } else {
            $this->session->set_flashdata('warning', 'Data didnt updated..!');
         redirect(base_url() . "index.php/purchase/purchase_history");
